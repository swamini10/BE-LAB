...
# Algorithm Steps:
# Input the number of items n and their weights and values.
# Input the capacity of the knapsack W.
...
# Add its full value to total_value.
# Subtract its weight from capacity.
# Else (if only part of it fits):
# Add the fractional value proportional to remaining capacity.
# Set capacity = 0 (knapsack is full).
# Stop when the knapsack is full or all items are considered.
# Output the total_value